...
export default function BoardView({ projectId }: BoardProps) {
  const { tasks,addTask, setTasks, updateTaskStatus, isLoading, setLoading, setError } =
    useAppStore();
      ...
      .then((response) => {
        ...
        if (response.success && response.data) {
          ...
          const updatedTasks = tasks.map((task) =>
            task.id === taskId
              ? { ...task, ...response.data, status: toStatus }
              : task
          );
...
const handleOnTaskCreated = (newTask: any) => {
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="grid grid-cols-1 gap-1 p-6 md:grid-cols-2 xl:grid-cols-4 bg-white min-h-screen custom-scrollbar">
        ...
        <div className="fixed bottom-4 right-4 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 shadow-sm">
          <div className="flex items-center space-x-2 text-blue-700">
            ...
            <span className="text-sm font-medium">
              Syncing {updateQueue.size} tasks...
            </span>
          </div>
        </div>
      )}
