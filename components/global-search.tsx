...
import { Search, FileText, User, Folder, Clock, Star, TrendingUp, File } from "lucide-react"
...
import type { SearchResults, Task, Document } from "@/lib/types"
...
export default function GlobalSearch({ onTaskClick }: GlobalSearchProps) {
  ...
  const shouldShowLoading = isLoading && debouncedQuery.trim().length >= 2
                    ...
                    <div className="p-2 border-t border-gray-100">
                      ...
                      {results.documents.map((document) => (
                        <div
                          ...
                          className="flex items-center space-x-3 px-3 py-3 hover:bg-emerald-50 cursor-pointer rounded-lg transition-colors group"
                        >
                          <div className="w-8 h-8 bg-emerald-100 rounded-lg flex items-center justify-center group-hover:bg-emerald-200 transition-colors">
                            <span className="text-sm">{getFileIcon(document.fileType)}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-medium font-medium text-gray-900 truncate">{document.name}</p>
                            <p className="text-small text-gray-500 truncate">
                              {document.fileName} • {formatFileSize(document.fileSize)}
                            </p>
                            {(document.project || document.task) && (
                              <div className="flex items-center space-x-2 mt-1">
                                {document.project && (
                                  <Badge variant="secondary" className="text-extra-small">
                                    📁 {document.project.name}
                                  </Badge>
                                )}
                                {document.task && (
                                  <Badge variant="outline" className="text-extra-small">
                                    ✅ {document.task.title}
                                  </Badge>
                                )}
                              </div>
                          ...
                          {document.uploader && (
                            <div className="flex items-center space-x-1">
                              ...
                              <span className="text-extra-small text-gray-400">{document.uploader.username}</span>
                            </div>
                          )}
                        </div>
                      ))}
