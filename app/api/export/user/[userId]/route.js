...
export async function POST(request, { params }) {
  try {
    ...
    const currentUserMember = await getWorkspaceMember(user.userId, currentWorkspaceId)
    ...
    const targetMember = workspace?.members.find((member) => member.memberId === userId)
    ...
    const filteredTasks = startDate ? allTasks.filter((task) => new Date(task.updatedAt) >= startDate) : allTasks
    ...
    const filteredProjects = startDate
      ? allProjects.filter((project) => new Date(project.updatedAt || project.createdAt) >= startDate)
      : allProjects
    ...
    const timeEntries = (await timeEntriesCollection.find(timeEntriesQuery).toArray()) 
    if (!timeEntries){
      return NextResponse.json({ success: false, error: "timeEntries not found" }, { status: 404 })
    }
    // Calculate statistics
    const completedTasks = filteredTasks.filter((task) => task.status === "Completed")
    const inProgressTasks = filteredTasks.filter((task) => task.status === "In Progress")
    const activeProjects = filteredProjects.filter((project) => project.status === "ongoing")

    const totalTimeTracked = timeEntries.reduce((total, entry) => total + (entry.duration || 0), 0)
    const filteredHours = totalTimeTracked / 3600

    // Get project names for tasks
    const projectIds = [...new Set(filteredTasks.map((task) => task.projectId).filter(Boolean))]
