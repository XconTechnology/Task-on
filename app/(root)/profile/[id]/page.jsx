...
export default function ProfilePage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const foundUser = membersRes.data.find(
  ...
  const loadFilteredData = async () => {
    try {
      ...
      const timeStatsRes = await timeTrackingApi.getStatsWithTimeframe(
        timeframe,
        userId
      );
      ...
      if (tasksRes.success && tasksRes.data) {
        ...
        if (reset) {
          ...
          setStats((prev) => ({
            ...
            completedTasks: tasksRes.data.filter(
              (t) => t.status === "Completed"
            ).length,
            inProgressTasks: tasksRes.data.filter(
              (t) => t.status === "In Progress"
            ).length,
            completionRate:
              tasksRes.data.length > 0
                ? Math.round(
                    (tasksRes.data.filter((t) => t.status === "Completed")
                      .length /
  ...
  const loadTimeEntries = async (page, reset = false) => {
    ...
    try {
      const timeRes = await timeTrackingApi.getFilteredUserTimeEntries(
        userId,
        timeframe,
        page,
        10
  ...
  const { targetRef: tasksTargetRef } = useInfiniteScroll(
    loadMoreTasks,
    tasks.hasMore,
    tasks.loading
  );
